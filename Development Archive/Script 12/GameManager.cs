...
public class GameManager : MonoBehaviour
{
    ...
    [Header("Day")]
    [Tooltip("Length of one work day, in seconds (180 = 3 minutes).")]
    ...
    [Tooltip("Each purchase multiplies that upgrade's cost by this.")]
    [SerializeField] private float costGrowth = 1.3f;
    [SerializeField] private int maxLevel = 8;
    [Tooltip("Carry-slot upgrade has its own cap (bag can't grow forever).")]
    ...
    // Upgrade levels: 0 = health, 1 = speed, 2 = tip, 3 = slots.
    private readonly int[] levels = new int[4];

    private GUIStyle big, mid, small, button, timerStyle;
    ...
    private void Start()
    {
        ...
        Time.timeScale = 0f; // start paused on the title screen
    }
    ...
    private void Update()
    {
        ...
        dayTimer -= Time.deltaTime;   // scaled: freezes while the job board is open
        if (dayTimer <= 0f)
        {
            dayTimer = 0f;
            phase = Phase.Shop;
    ...
    {
        float baseCost = i == 0 ? healthBaseCost
                       : i == 1 ? speedBaseCost
    ...
    private void DrawStart()
    {
        ...
        Centered(y, 900, "Work the day, bank the cash, buy upgrades. Don't crash.", mid, Color.white); y += 60;

        Centered(y, 900, "WASD / Arrows  \u2014  Drive", small, new Color(0.85f, 0.88f, 0.92f)); y += 30;
        Centered(y, 900, "Tab  \u2014  Open the job board (accept deliveries)", small, new Color(0.85f, 0.88f, 0.92f)); y += 30;
        Centered(y, 900, "Green beam = go collect     Blue beam = go deliver", small, new Color(0.85f, 0.88f, 0.92f)); y += 30;
        Centered(y, 900, "M  \u2014  Change radio station", small, new Color(0.85f, 0.88f, 0.92f)); y += 30;
        Centered(y, 900, "Watch for traffic \u2014 getting hit hurts!", small, new Color(1f, 0.7f, 0.4f)); y += 50;

        if (Button(cx - 120, y, 240, 56, "START DAY 1")) BeginDay();
    }
    ...
    private void DrawGameOver()
    {
        ...
        Centered(y, 600, "YOU CRASHED OUT", big, new Color(0.95f, 0.3f, 0.3f)); y += 80;

        float money = delivery != null ? delivery.Money : 0f;
        int deliveries = delivery != null ? delivery.DeliveriesCompleted : 0;
        Centered(y, 600, $"Survived {day} day(s)", mid, Color.white); y += 44;
        Centered(y, 600, $"Deliveries: {deliveries}", small, new Color(0.85f, 0.87f, 0.9f)); y += 34;
        Centered(y, 600, $"Cash: ${money:0.00}", small, new Color(0.4f, 0.9f, 0.5f)); y += 60;

        if (Button(cx - 120, y, 240, 56, "NEW RUN")) Restart();
    }
